...
private:
    ...
    std::array<GameObject, MAX_OBJECTS> _gameObjects;
...
private:
    ...
    {
        _gameObjects[0].position = {0.0f, 0.0f, 0.0f};
        _gameObjects[0].rotation = {0.0f, 0.0f, 0.0f};
        _gameObjects[0].scale = {1.0f, 1.0f, 1.0f};

        _gameObjects[1].position = {-2.0f, 0.0f, -1.0f};
        _gameObjects[1].rotation = {0.0f, glm::radians(45.0f), 0.0f};
        _gameObjects[1].scale = {0.75f, 0.75f, 0.75f};

        _gameObjects[2].position = {2.0f, 0.0f, -1.0f};
        _gameObjects[2].rotation = {0.0f, glm::radians(-45.0f), 0.0f};
        _gameObjects[2].scale = {0.75f, 0.75f, 0.75f};
    }
    ...
    void UpdateUniformBuffer(uint32_t frameIndex)
    {
        ...
        ubo.view = glm::lookAt(glm::vec3(2.0f, 2.0f, 6.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        ubo.projection = glm::perspective(glm::radians(45.0f), static_cast<float>(_swapChainExtent.width) / static_cast<float>(_swapChainExtent.height), 0.1f, 20.0f);
        ...
        for (auto& object : _gameObjects)
        {
            ...
            glm::mat4 model = object.GetModelMatrix() * initialRotation;

            ubo.model = model;

            memcpy(object.uniformBuffersMapped[frameIndex], &ubo, sizeof(ubo));
        }
    }
    ...
    void Cleanup()
    {
        ...
        _device.freeDescriptorSets(_descriptorPool, _descriptorSets.size(), _descriptorSets.data());
        _device.destroyDescriptorPool(_descriptorPool);

        _device.destroyBuffer(_indexBuffer);
        _device.freeMemory(_indexBufferMemory);

        CleanupColorResouces();
        CleanupDepthResouces();

        for (auto uniformBuffer : _uniformBuffers)
            _device.destroyBuffer(uniformBuffer);

        for (auto bufferMemory : _uniformBufferMemory)
        {
...
private:
    ...
    void CreateLogicalDevice()
    {
        ...
        for (uint32_t qfpIndex = 0; qfpIndex < queueFamilyProperties.size(); ++qfpIndex)
        {
            ...
            if ((queueFamilyProperties[qfpIndex].queueFlags & vk::QueueFlagBits::eTransfer) && !(queueFamilyProperties[qfpIndex].queueFlags & vk::QueueFlagBits::eGraphics) && !transferFound)
            {
        ...
        std::set uniqueQueues = { _graphicsQueueIndex, _transferQueueIndex };
    ...
    void CleanupColorResouces()
    {
        _device.destroyImage(_colorImage);
        _device.freeMemory(_colorImageMemory);
    ...
    void CreateImageViews()
    {
        ...
        for (const auto& image : _swapChainImages)
            _swapChainImageViews.emplace_back(CreateImageView(image, _surfaceFormat.format, vk::ImageAspectFlagBits::eColor, 1));
    }
    ...
    void CreateDescriptorSetLayout()
    {
        ...
        uboLayoutBindings[1].stageFlags = vk::ShaderStageFlagBits::eFragment;
    ...
    void CreateColorResouces()
    {
        ...
        std::tie(_colorImage, _colorImageMemory) = CreateImage(
    ...
    void CreateDepthResources()
    {
        ...
        _depthImageView = CreateImageView(_depthImage, format, vk::ImageAspectFlagBits::eDepth, 1);
    }
    ...
    void CreateTextureImage()
    {
        ...
        std::tie(_textureImage, _textureImageMemory) = CreateImage(texWidth, texHeight,
                                                                         _mipLevels,
        ...
        TransitionImageLayout(commandBuffer, _textureImage, vk::ImageAspectFlagBits::eColor, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal,
            vk::PipelineStageFlagBits2::eNone, {}, vk::PipelineStageFlagBits2::eTransfer, vk::AccessFlagBits2::eTransferWrite, 0, _mipLevels);

        CopyBufferToImage(commandBuffer, stagingBuffer, _textureImage, texWidth, texHeight);

//        TransitionImageLayout(commandBuffer, _textureImage, vk::ImageAspectFlagBits::eColor, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal,
//        vk::PipelineStageFlagBits2::eTransfer, vk::AccessFlagBits2::eTransferWrite, vk::PipelineStageFlagBits2::eFragmentShader, vk::AccessFlagBits2::eShaderRead, 0, _mipLevels);

        GenerateMipmaps(commandBuffer, _textureImage, vk::Format::eR8G8B8A8Srgb, texWidth, texHeight, _mipLevels);
    ...
    void GenerateMipmaps(vk::CommandBuffer commandBuffer, vk::Image image, vk::Format imageFormat, uint32_t texWidth, uint32_t texHeight, uint32_t mipLevels)
    {
        ...
        for (uint32_t i = 1; i < mipLevels; i++)
        {
            ...
            vk::ImageBlit blit {};
            blit.srcSubresource = {.aspectMask = vk::ImageAspectFlagBits::eColor, .mipLevel = i - 1, .layerCount = 1};
            blit.dstSubresource = { .aspectMask = vk::ImageAspectFlagBits::eColor, .mipLevel = i, .layerCount = 1 };
            ...
            blit.dstOffsets = std::array<vk::Offset3D, 2> {{
                vk::Offset3D {0, 0, 0},
                vk::Offset3D {1 < mipWidth ? mipWidth / 2 : 1, 1 < mipHeight ? mipHeight / 2 : 1, 1}
            }};
    ...
    {
        _textureImageView = CreateImageView(_textureImage, vk::Format::eR8G8B8A8Srgb, vk::ImageAspectFlagBits::eColor, _mipLevels);
    }
    ...
    vk::ImageView CreateImageView(vk::Image image, vk::Format format, vk::ImageAspectFlagBits aspectFlags, uint32_t mipLevels)
    {
        ...
        createInfo.subresourceRange = { .aspectMask = aspectFlags, .baseMipLevel = 0, .levelCount = mipLevels, .baseArrayLayer = 0, .layerCount = 1 };

        return _device.createImageView(createInfo);
    }

    std::pair<vk::Image, vk::DeviceMemory> CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, vk::SampleCountFlagBits numSamples, vk::Format format, vk::ImageTiling tiling, vk::ImageUsageFlags usage, vk::MemoryPropertyFlags properties)
    {
        ...
        vk::Image image = _device.createImage(createInfo);
        ...
        vk::DeviceMemory imageMemory = _device.allocateMemory(allocInfo);
    ...
    void CopyBufferToImage(vk::CommandBuffer commandBuffer, vk::Buffer buffer, vk::Image image, uint32_t width, uint32_t height)
    {
        ...
        region.imageSubresource = {.aspectMask = vk::ImageAspectFlagBits::eColor, .mipLevel = 0, .baseArrayLayer = 0, .layerCount = 1};
    ...
    void CreateUniformBuffers()
    {
        for (auto& object : _gameObjects)
        {
            ...
            for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; ++i)
            {
                ...
                object.uniformBuffers.emplace_back(buffer);
                object.uniformBuffersMemory.emplace_back(bufferMemory);
                object.uniformBuffersMapped.emplace_back(_device.mapMemory(object.uniformBuffersMemory[i], 0, bufferSize));
            }
        }
    }
    ...
    void CreateDescriptorPool()
    {
        ...
        createInfo.maxSets = MAX_FRAMES_IN_FLIGHT * MAX_OBJECTS;
    ...
    void CreateDescriptorSets()
    {
        for (auto& object : _gameObjects)
        {
            ...
            object.descriptorSets = _device.allocateDescriptorSets(allocInfo);
    ...
    {
        vk::CommandBuffer commandCopyBuffer = BeginSingleTimeCommands(_transferCommandPool);
        commandCopyBuffer.copyBuffer(src, dst, vk::BufferCopy(0, 0, size));
        EndSingleTimeCommands(commandCopyBuffer, _transferQueue);
    }
    ...
    void RecordCommandBuffer(uint32_t imageIndex) const
    {
        ...
        colorAttachmentInfo.imageView = _colorImageView;
        colorAttachmentInfo.imageLayout = vk::ImageLayout::eColorAttachmentOptimal;
        colorAttachmentInfo.resolveMode = vk::ResolveModeFlagBits::eAverage;
        colorAttachmentInfo.resolveImageView = _swapChainImageViews[imageIndex];
        ...
        {
            _commandBuffers[_frameIndex].bindDescriptorSets(
                vk::PipelineBindPoint::eGraphics,
                _pipelineLayout,
                0,
                object.descriptorSets[_frameIndex],
                nullptr
            );
